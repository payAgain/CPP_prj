...
static ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_worker_config
    = ConfigMgr::getInstance()->look_up("workers", "worker config", std::map<std::string, std::map<std::string, std::string> >());
...
TimedWorkerGroup::ptr TimedWorkerGroup::Create(uint32_t batch_size, uint32_t wait_ms, IOManager* s) {
...
void TimedWorkerGroup::start() {
    m_timer = IOManager::GetThis()->addTimer(m_waitTime, std::bind(&TimedWorkerGroup::onTimer, shared_from_this()));
...
TimedWorkerGroup::TimedWorkerGroup(uint32_t batch_size, uint32_t wait_ms, IOManager* s)
    :m_batchSize(batch_size)
    ,m_finish(false)
    ,m_timedout(false)
    ,m_waitTime(wait_ms)
...
Scheduler::ptr WorkerManager::get(const std::string& name) {
    ...
    return it->second[Atomic::addFetch(s_count) % it->second.size()];
}
...
bool WorkerManager::init() {
    auto workers = g_worker_config->get_value();
    return init(workers);
}
