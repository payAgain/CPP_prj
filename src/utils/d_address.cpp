...
static Logger::Logger::ptr g_logger = DREAMER_SYSTEM_LOGGER();
...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
    ...
    if(error) {
        D_SLOG_ERROR(g_logger) << "Address::Lookup getaddress(" << host << ", "
    ...
    while(next) {
        ...
        //D_SLOG_INFO(g_logger) << ((sockaddr_in*)next->ai_addr)->sin_addr.s_addr;
        next = next->ai_next;
    }
...
bool Address::GetInterfaceAddresses(std::multimap<std::string
    ...
    if(getifaddrs(&results) != 0) {
        D_SLOG_ERROR(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
    ...
    try {
        ...
        D_SLOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
    ...
    if(error) {
        D_SLOG_ERROR(g_logger) << "IPAddress::Create(" << address
            << ", " << port << ") error=" << error
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
    ...
    if(result <= 0) {
        D_SLOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", "
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
    ...
    if(result <= 0) {
        D_SLOG_ERROR(g_logger) << "IPv6Address::Create(" << address << ", "
                << port << ") rt=" << result << " errno=" << errno
                << " errstr=" << strerror(errno);
        return nullptr;
    }
...
uint32_t IPv6Address::getPort() const {
    return htobe64(m_addr.sin6_port);
}
